...
const PasscodeSetupPage = () => {
  ...
  const userDOB = localStorage.getItem("userDOB"); // Assuming DOB is stored as a string

  // Helper function to check if the passcode is related to the DOB
  const isPasscodeRelatedToDOB = (passcode, dob) => {
    // Check if the passcode contains any part of the user's DOB
    if (dob && passcode.includes(dob)) {
      return true;
    }
    return false;
  };
  ...
  const handleProceed = () => {
    ...
    if (isPasscodeRelatedToDOB(passcode, userDOB)) {
      setError("Passcode cannot be related to your Date of Birth.");
      return;
    }
    if (passcode.length !== 6 || isNaN(passcode)) {
      setError("Passcode must be exactly 6 digits.");
      return;
    }

    // If passcode is valid, set the redirect state and navigate to Biometrics Setup
    setIsRedirecting(true);
    setTimeout(() => {
      navigate("/biometrics-setup");
    }, 2000); // Wait for 2 seconds to show the redirect message
  };
    ...
    >
      ...
      <div className="bg-black bg-opacity-90 border border-white border-opacity-20 rounded-xl shadow-lg w-[90%] max-w-[900px] flex flex-col justify-between mt-12 sm:mt-24 mx-auto px-4 py-6">
        ...
        <div className="flex w-full mb-6 gap-x-2">
          <button
            className="w-1/2 border border-white text-white font-semibold py-2 px-6 rounded-l hover:bg-gray-800"
            onClick={() => navigate("/account-setup")}
          >
            PREVIOUS
          </button>
          <button
            className="w-1/2 bg-yellow-500 hover:bg-yellow-600 text-black font-semibold py-2 px-6 rounded-r"
      ...
      {isRedirecting && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-xl font-bold">
          Redirecting to Biometrics Setup...
        </div>
      )}
    </div>
  );
