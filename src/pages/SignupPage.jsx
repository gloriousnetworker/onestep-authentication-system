...
const SignupPage = ({ showNotification }) => {
  ...
  // Generate OTP, show "Redirecting to Telegram...", and navigate to grant-access-signup page
  const handleGenerateOTP = async () => {
    ...
    setTimeout(async () => {
      try {
        await login();  // Trigger the login function to authenticate the user
        localStorage.setItem("hasPassedSignup", "true");  // Set the flag on successful signup
        showNotification("Welcome to Onestep Authentication System!", "success");
      ...
      <div className="bg-black bg-opacity-90 border border-white border-opacity-20 rounded-xl shadow-lg w-[90%] max-w-[900px] flex flex-col justify-between mt-12 sm:mt-24 mx-auto px-4 py-6">
        ...
        <div className="flex flex-col justify-center items-center mb-4 mt-6 cursor-pointer">
          ...
          <img
            ...
            loading="lazy"
            onClick={handleGenerateOTP}  // Clicking the logo generates OTP and triggers signup/login
          />
          {redirecting && (
