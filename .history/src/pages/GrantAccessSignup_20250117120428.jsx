...
const GrantAccessSignup = ({ showNotification }) => {
  ...
  const handleSendMessage = () => {
    ...
    setIsLoading(false); // Reset loading state after the OTP process is complete
  };
  ...
  const handleCopyOtp = () => {
    if (otp) {
      navigator.clipboard.writeText(otp).then(() => {
        ...
        navigate("/otp-verification-signup"); // Redirect to OTPVerification page
      });
    }
  };
    ...
    >
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-10">
        <div className="bg-black bg-opacity-90 text-white rounded-xl shadow-lg w-full max-w-md p-6 text-center">
          ...
          >
            {isLoading ? "Requesting OTP..." : "SEND MESSAGE"} {/* Show loading message */}
          </button>
...
export default GrantAccessSignup;
