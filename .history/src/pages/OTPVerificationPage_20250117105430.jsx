...
const OTPVerificationPage = ({ showNotification }) => {
  ...
  const handleResendOtp = () => {
    ...
    showNotification("Your new OTP has been sent. Check your Telegram account.", "info");
  ...
  const handleProceed = () => {
    ...
    setTimeout(() => {
      if (enteredOtp === storedOtp && timer > 0) {
        showNotification("OTP verified successfully! Redirecting to Dashboard...", "success");
        navigate("/dashboard");
      } else {
        showNotification("Invalid OTP or OTP has expired. Please try again.", "error");
      }
      setLoading(false);
    }, 1000);
  };
