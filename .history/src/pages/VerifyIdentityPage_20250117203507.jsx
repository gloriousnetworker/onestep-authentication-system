...
const VerifyIdentityPage = () => {
  ...
  const generateOSID = () => {
    // Generate a unique OS-ID (you can customize this format)
    return `OS-${Math.random().toString(36).substr(2, 9).toUpperCase()}-${Date.now()}`;
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (formData.identityNumber && formData.identityProof) {
      // Generate and store the OS-ID in local storage
      const osId = generateOSID();
      localStorage.setItem("os-id", osId);

      // Log the OS-ID for debugging purposes
      console.log("Generated OS-ID:", osId);
